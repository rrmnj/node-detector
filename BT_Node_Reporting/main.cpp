...
string determine_status(Node sender) {
	string status;
		...
		if (sender.get_notification().find("LOST") != std::string::npos)	//if the sender notification contains the words "LOST"
		{
			status = ("DEAD");	//we set the friend node status to dead
		}
		if (sender.get_notification().find("FOUND") != std::string::npos)	//if the sender says the node is found
		{
			status = ("ALIVE");	//set to alive ..
		}
	return status;
}
...
void update_node(Node sender, char element, vector<Node>& vectorOfNodes, string status) {
	if (vectorOfNodes[element].get_generated_time() < sender.get_generated_time())  		//updates on node only performed if the notification is newer than the notification currently stored
	{ 
		...
		//this if statement is here to catch disparties between the received and generated time. In order to determine ambuigity, this function looks for patterns where the generated time is <= 50,
		...
		vectorOfNodes[element].set_notification(sender.get_notification());	//set to update + friend name
...
void add_friend_node(Node sender, vector<Node>& vectorOfNodes)
{
	...
	sender.get_friend_name(), sender.get_notification(), sender.get_name());
...
void parse_input(string line, vector<Node>& vectorOfNodes) {
	string temp, notification;	//temporarily stores the string through >>, to be then pushed into the vector
	...
	if (input[3] == "HELLO")
	{
		notification = input[2] + " " + input[3]; //two arguments
		//this was necessary as an error would be thrown if I tried to pass input[4] whilst it was empty
		Node newNode(received, generated, input[2], notification, "");
		analyse_node(newNode, vectorOfNodes);
	}
	else
	{
		notification = input[2] + " " + input[3] + " " + input[4]; //three arguments
		Node newNode(received, generated, input[2], notification, input[4]);
		analyse_node(newNode, vectorOfNodes);
	}
}